...
function CommunityList() {
  ...
  const uniqueLocations = [...new Set(community.map((item) => item.location))];
  ...
  const currentItems = filteredCommunity.slice(indexOfFirstItem, indexOfLastItem);
  const totalPages = Math.ceil(filteredCommunity.length / itemsPerPage);

  const startIndex = indexOfFirstItem + 1;
  const endIndex = Math.min(indexOfLastItem, filteredCommunity.length);
  ...
  return (
    <div className="min-h-screen">
      ...
      <div
        ...
        style={{
          backgroundImage: `url('https://images.unsplash.com/photo-1603791440384-56cd371ee9a7?ixlib=rb-4.0.3&auto=format&fit=crop&w=2073&q=80')`,
        }}
      >
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center text-white px-4 z-10">
            ...
            <p className="text-lg md:text-xl mb-8 max-w-2xl mx-auto leading-relaxed">
              Connect with neighbors and join local events.
            </p>
          </div>
        </div>
          ...
          <div className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <p className="text-gray-600 mt-1">
                Showing {startIndex}-{endIndex} of {filteredCommunity.length} events
              </p>
            </div>
